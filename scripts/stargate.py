...
topics = ["0xe9bded5f24a4168e4f3bf44e00298c993b22376aad8c58c7dda9718a54cbea82"]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "result" in response_json:
                ...
                for log in logs:
                    data = log["data"]
                    recipient = "0x" + data[1194:1234]  # extract recipient from data
                    tx_hash = log["transactionHash"]
                    result.append((recipient, tx_hash))
                block_number += 10000
                with open('stargate_eth.json', 'w') as f:
                    json.dump(result, f)
                print(f"Saved batch to stargate.json, block number: {block_number}")
            else:
                print("Error:", response_json)
        else:
