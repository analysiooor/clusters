...
topics = ["0xed8e6ba697dd65259e5ce532ac08ff06d1a3607bcec58f8f0937fe36a5666c54"]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "result" in response_json:
                ...
                for log in logs:
                    data = log["data"]
                    recipient = "0x" + data[602:642]  # extract recipient from data
                    tx_hash = log["transactionHash"]
                    result.append((recipient, tx_hash))
                block_number += 10000
                with open('connext_eth.json', 'w') as f:
                    json.dump(result, f)
                print(f"Saved batch to connext.json, block number: {block_number}")
            else:
                print("Error:", response_json)
        else:
